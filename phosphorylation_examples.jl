...
for k = 1:5
    ...
    if k<=2
        t_grc = @elapsed grc = generic_root_count(F, check_cotransversality=false).count
    ...
    t_toric = @elapsed grc_toric = toric_root_bound(A, F, check_cotransversality=false).bound
    ...
    t_grc_mat = @elapsed grc_mat = generic_root_count(F, check_cotransversality=true).count
    println("Generic root count with transversality: $grc_mat (computed in $t_grc_mat seconds)")
    t_toric_mat = @elapsed grc_toric_mat = toric_root_bound(A, F, check_cotransversality=true).bound
